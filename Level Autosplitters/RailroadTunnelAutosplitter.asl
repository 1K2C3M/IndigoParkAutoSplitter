...
update {
  if (current.loading == 1 && current.scene == "/RailroadTunnel") {
    vars.startTimer = 1;
  }
}

start {
  if (current.scene == "/RailroadTunnel" && (current.moving != 0 || current.moving == null) && vars.startTimer == 1) {
    vars.startTimer = 0;
...
split {
  if (current.scene != old.scene && old.scene == "/RailroadTunnel") {
    return true;
  }
  return false;
}
